...
9. Begin Phase 5 — the REST API. DRF viewsets/serializers for courses, sections, terms, and enrollments; token auth for mobile; structured `{"rule": "R5", "detail": "..."}` errors on registration/drop failures; permission classes enforcing that a student only ever sees their own enrollments.
